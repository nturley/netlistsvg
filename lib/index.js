...
function render(netlistpath, outputPath, skinPath)
{
    ...
    console.log('skin file: ' + skinPath);
    fs.readFile(skinPath, 'utf-8', function(e, d) {skin_read(e,d,netlistpath, outputPath)});
}

function skin_read(err, data, netlistpath, outputPath) {
    if (err) throw err;
    var skin = onml.p(data);
    console.log('netlist file: ' + netlistpath)
    fs.readJson(netlistpath, function (err, yosys_netlist) {
        ...
        klay.layout({
            ...
            success: function(g) { klayed_out(g, module, skin, outputPath)},
