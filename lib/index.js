...
function render(yosys_netlist, // parsed json : object
                skin_data, //skin data : string
                done) // f(error, result)
{
    // use onml to parse the skin XML
    var skin = onml.p(skin_data);
    // Find top module in the netlist
    ...
    klay.layout({
        ...
        success: function(g) {
            var result = klayed_out(g, module, skin);
            done(null, result);
...
// extract the klay layput properties from the skin
...
// call back function after layout is complete
// takes template from skin, takes position from kgraph
...
function klayed_out(g, module, skin_data) {
    var nodes = _.map(module.nodes, function(n){
        ...
        // these nodes have the node id as the text field value
    ...
    // copy the root attributes from the skin
    var svg = skin_data.slice(0,2);
    svg[1].width = g.width;
    svg[1].height = g.height;
    // copy the style nodes
    var styles = _.filter(skin_data, function (el) {
        return el[0] == 'style';
    });
    // combine skin header with styles, nodes, and lines and write to output file
...
function buildKGraph(module, module_name, skin_data)
{
    ...
    var edges = _.flatMap(module.wires,function(w) {
        ...
        return _.map(w.riders, function (r) {
            ...
            // DFF's are given a lower priority than the other nodes
