...
function render(skinPath, netlistpath)
{
    fs.readFile(skinPath, 'utf-8', function(e, d) {skin_read(e,d,netlistpath)});
}

function skin_read(err, data, netlistpath) {
    if (err) throw err;
    console.log('skin file read.');
    var skin = onml.p(data);
    fs.readJson(netlistpath, function (err, yosys_netlist) {
        if (err) throw err;
        console.log('netlist file read.');
        ...
        klay.layout({
            graph: kgraph,
            options: { spacing: 25, borderSpacing: 50, nodeLayering: 'LONGEST_PATH' },
            success: function(g) { klayed_out(g, module, skin)},
            error: function(error) { if (error) throw error; }
        });
...
function klayed_out(g, module, skin_data) {
    ...
    fs.writeFile('out.svg', onml.s(ret), 'utf-8', function(e) { if (e) throw e;});
}
