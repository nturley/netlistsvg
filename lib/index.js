...
function buildKGraph(module, module_name, skin_data)
{
    ...
    var edges = _.flatMap(module.wires,function(w) {
        ...
        // at least two drivers and no riders
            ...
            // connect driver[0] to the input side
            var driver = w.drivers[0];
            var sourceParentKey = driver.parentNode.key;
            var dummyEdges = [{
                id: 'e' + i,
                source: sourceParentKey,
                ...
                target: dummyId,
                targetPort: dummyId+'.A'
            }];
            i += 1;
            if (driver.parentNode.type != '$dff') {
                dummyEdges[0].layoutOptions = {'org.eclipse.elk.layered.priority.direction':10};
            }
            // connect driver[1:N] to the output side
            var moreEdges = _.map(w.drivers.slice(1), function(driver) {
                var sourceParentKey = driver.parentNode.key;
                return {
                    id: 'e' + i,
                    source: sourceParentKey,
                    sourcePort: sourceParentKey + '.' + driver.key,
                    target: dummyId,
                    targetPort: dummyId + '.B'
                };
            });
            return dummyEdges.concat(moreEdges);
        // at least one rider and no drivers
        } else if (w.riders.length > 1 && w.drivers.length == 0) {
            // create a dummy node and add it to children
            var dummyId = addDummy(children, dummies);
            dummies += 1;
            // connect rider[0] to the output side
            var rider = w.riders[0];
            var sourceParentKey = rider.parentNode.key;
            var dummyEdges = [{
                id: 'e' + i,
                source: sourceParentKey,
                sourcePort: sourceParentKey + '.' + rider.key,
                target: dummyId,
                targetPort: dummyId + '.B'
            }];
            i += 1;
            // connect rider[1:N] to the output side
            var moreEdges = _.map(w.riders.slice(1), function(rider) {
                var sourceParentKey = rider.parentNode.key;
                var edge = {
                    id: 'e' + i,
                    source: sourceParentKey,
