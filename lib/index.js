...
function render(skin_data, yosys_netlist, done) {
    ...
    const components = module.nodes.filter(n => n.type === 'r' || n.type === 'c' || n.type === 'd_led').map(n => n.key)
    ...
    const promises = verticalNodeCombinations.map(verticals => {
        ...
        for (const n of m.nodes) {
            if (n.type === 'r' || n.type === 'c' || n.type === 'd_led') {
    ...
    const promise = Promise.all(promises).then(graphs => {
        ...
        console.log('<div style="display:flex;flex-wrap:wrap">')
